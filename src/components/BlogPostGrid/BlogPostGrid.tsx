...
export const BlogPostGrid: React.FC<BlogPostGridProps> = ({
    ...
    const [hasBlogPermission, setHasBlogPermission] = useState(false);

    const createCardShown = showCreateCard && (currentPage === 1) && hasBlogPermission;
    ...
    const checkBlogPermission = useCallback<() => Promise<boolean>>(async () => {
        // If no organization, don't check permissions
        if (!organization) return false;

        // Check if user is logged in
        if (!currentUser) return false;

        // Check if user is executive or owner
        if (currentUser?.executive || currentUser?.owner) return true;

        // For non-executive/non-owner users, check specific blog permissions
        try {
            const permissionsRef = doc(db, 'UserPermissions', currentUser.uid);
            ...
            if (permissionsDoc.exists()) {
                ...
                return data?.permissions?.[`${organization}Blog`] === true;
            } else {
                notifications.show({
                    title: 'Error checking permissions',
                    message: 'No permissions document found',
                    color: 'red'
                });
            }
        } catch (error) {
    ...
    const fetchPosts = useCallback(async () => {
        ...
        try {
            ...
            if (showCreateCard) {
                const hasPermission = await checkBlogPermission();
                setHasBlogPermission(hasPermission);
            }
            ...
            const numPostsToFetch = createCardShown ? postsPerPage - 1 : postsPerPage;
            const postsToSkip = pagesToSkip * numPostsToFetch;

            // Get all posts up to the current page
            let postsQuery = query(
                filteredQuery,
                orderBy('timestamp', 'desc'),
                limit(postsToSkip + numPostsToFetch)
            );
            ...
            // Only fetch author info if showAuthorInfo is true
            if (showAuthorInfo) {
    ...
    }, [db, organization, currentPage, showInvisiblePosts, createCardShown, showAuthorInfo, checkBlogPermission, showCreateCard]);
                        ...
                        title={
                            <Stack gap="5">
                                <Group>
                                    <Text
                                        variant="gradient"
                                        gradient={committeeGradient}
                                        fw={1000}
                                        size="xl"
                                    >
                                        {committeeName} Update
                                    </Text>
